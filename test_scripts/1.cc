...
int main() {
    ...
    pointToPoint.SetDeviceAttribute("DataRate", DataRateValue(DataRate("0.1Kbps")));
    ...
    uint16_t port = 9;
    Address serverAddress(InetSocketAddress(interfaces.GetAddress(21), port));
    BulkSendHelper serverHelper("ns3::TcpSocketFactory", serverAddress);
    ApplicationContainer serverApps = serverHelper.Install(nodes.Get(21));
    ...
    clientHelper.SetAttribute("MaxBytes", UintegerValue(0)); // Set MaxBytes to 0 for unlimited bytes
